...
// SEE: https://github.com/bluesky-social/social-app/blob/main/src/locale/languages.ts#L17
...
function convert (languages: Array<Language>): Array<TTOption> {
  return languages

    // ISO 3166-1 alpha-2 による重複言語の削除
    .filter((language1: Language, index: number) => {
      return languages.findIndex((language2: Language) => {
        return language1.code2 === language2.code2
      }) === index
    })

    // ISO 3166-1 alpha-2 が未指定の言語を削除
    .filter((language: Language) => {
      return !!language.code2.trim()
    })

    // TTOption 型に変換
    // ISO 3166-1 alpha-3 は使用しない
    .map((language: Language): TTOption => {
      return {
        label: language.name,
...
export default convert(LANGUAGES)
