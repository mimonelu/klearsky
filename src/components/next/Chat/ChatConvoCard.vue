...
async function acceptRequest () {
  ...
  const result = await props.myConvo.accept()
  mainState.loaderDisplay = false
  if (result) {
    mainState.openChatConvoPopup(props.myConvo)
  }
}
...
async function rejectRequest () {
  ...
  if (await mainState.openConfirmationPopup({
    ...
    mainState.loaderDisplay = true
    await props.myConvo.leave()
    mainState.loaderDisplay = false
  }
}

function openChatConvoPopup () {
  Util.blurElement()
  if (props.myConvo.data?.status === "accepted") {
    mainState.openChatConvoPopup(props.myConvo)
  }
}

function openChatConvoPopover ($event: Event) {
  Util.blurElement()
  mainState.chatConvoPopoverProps.myConvo = props.myConvo
...
<template>
  <div
    class="convo-card"
    :data-has-unread-messages="(myConvo.data?.unreadCount ?? 0) > 0"
    :data-muted="myConvo.data?.muted"
    :data-status="myConvo.data?.status"
    @click="openChatConvoPopup"
  >
    <!-- 未読メッセージ数 -->
    <div
    ...
    <div
      v-if="menuDisplay"
      class="convo-card__right"
    >
      <button
        class="button--plain"
        @click.stop="openChatConvoPopover($event)"
      >
        <SVGIcon name="menu" />
      </button>
    </div>
