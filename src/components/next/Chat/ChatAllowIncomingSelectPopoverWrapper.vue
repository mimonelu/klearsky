...
async function callback (allowIncoming: TTAllowIncoming) {
  Util.blurElement()
  close()
  mainState.loaderDisplay = true
  if (await mainState.myChat!.setAllowIncoming(allowIncoming)) {
    if (mainState.userProfile?.associated?.chat != null) {
      mainState.userProfile.associated.chat.allowIncoming = allowIncoming
    }
  }
...
<template>
  <button
    class="chat-allow-incoming-select-popover-wrapper menu-ticker__sub-trigger"
    ref="trigger"
    type="button"
    @click.prevent.stop
    @mouseenter="open"
    ...
    <SVGIcon :name="
      state.allowIncoming === 'all'
        ...
        : state.allowIncoming === 'following'
          ...
          : 'personOff'
    " />
    <span>{{ $t("chatAllowIncomingSelect") }}</span>
...
.chat-allow-incoming-select-popover-wrapper {
  &:deep() {
